...
`package.json` `packageManager`, currently `pnpm@10.34.5`). `nvm use` reads the
version out of `.nvmrc`. 22 is not a preference: the server's stores are built
on `node:sqlite`, which arrived in 22.5, and CI and the production image both
run 22.
