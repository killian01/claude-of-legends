...
// the forwarded chain as the operator says is trustworthy, and failing that
// the peer address the nearest proxy states in X-Real-IP.
export function clientAddress(
  ...
  // A proxy that states only the peer it saw writes X-Real-IP and leaves the
  // chain empty. Our own Caddy does exactly that whenever the CDN in front is
  // DNS-only: it fills X-Forwarded-For from the CDN's client header, which is
  // then absent, and X-Real-IP from the peer, which is the player. Read on the
  // same operator statement as the chain, never on a direct port.
  const stated = normalizeIp(first(headers['x-real-ip']));
