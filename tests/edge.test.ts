...
describe('client address', () => {
  ...
  it('reads the peer the proxy states when it forwards no chain', () => {
    // Our Caddy fills X-Forwarded-For from the CDN's client header and
    // X-Real-IP from the peer it saw. With the CDN DNS-only the first is
    // empty and the second is the player; without this every player would
    // land in Caddy's bucket and share one cap.
    const h = headers({ 'x-forwarded-for': '', 'x-real-ip': '1.2.3.4' });
    expect(clientAddress(h, PROXY, ONE_PROXY)).toBe('1.2.3.4');
    expect(clientAddress(headers({ 'x-real-ip': '1.2.3.4' }), PROXY, ONE_PROXY)).toBe('1.2.3.4');
  });

  it('prefers the forwarded chain to the stated peer', () => {
    // Behind the CDN the chain names the player and X-Real-IP names the CDN.
    const h = headers({ 'x-forwarded-for': '1.2.3.4', 'x-real-ip': '198.51.100.5' });
    expect(clientAddress(h, PROXY, ONE_PROXY)).toBe('1.2.3.4');
  });

  it('ignores a stated peer nobody vouched for', () => {
    const h = headers({ 'x-real-ip': '1.2.3.4' });
    expect(clientAddress(h, '203.0.113.9', DIRECT)).toBe('203.0.113.9');
  });

  it('never lets a junk stated peer become its own bucket', () => {
    expect(clientAddress(headers({ 'x-real-ip': 'not-an-ip' }), PROXY, ONE_PROXY)).toBe(PROXY);
    expect(clientAddress(headers({ 'x-real-ip': '::ffff:1.2.3.4' }), PROXY, ONE_PROXY)).toBe(
      '1.2.3.4',
    );
  });
});
