...
// the Warden slain; each with its tick and the team that scored it, so the
...
export type MarkKind = 'kill' | 'tower' | 'sanctum' | 'warden';
...
export class MarkCollector {
  ...
  note(units: ReadonlyMap<number, Unit>): void {
    for (const u of units.values()) {
      if (!this.seats.has(u.id)) this.seats.set(u.id, { kind: u.kind, team: u.team });
    }
  }
  ...
  observe(tick: number, events: readonly SimEvent[]): void {
    for (const ev of events) {
      ...
      const team: TeamId | null =
        killer && killer.kind !== 'warden' && killer.kind !== 'camp'
          ? killer.team
          : kind === 'tower' || kind === 'sanctum'
            ? ((1 - victim.team) as TeamId)
            : null;
      this.marks.push({ tick, kind, team, unitId: ev.unitId, killerId: ev.killerId });
    }
  }
}
