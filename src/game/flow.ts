...
export type GameMode = 'practice' | 'queue' | 'create' | 'join';
export type NextStep = 'home' | 'replay' | 'requeue';

// 'again' after practice replays the same offline pick without re-entering
// champion select; after ANY online mode it re-enters the public queue,
// because a private lobby is destroyed with its match and its code has
// nothing left to join.
export function nextStep(action: PostMatchAction, mode: GameMode): NextStep {
  if (action === 'menu') return 'home';
  return mode === 'practice' ? 'replay' : 'requeue';
}
