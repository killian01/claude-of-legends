...
// mage, an assassin or a battlemage), two top (a tank or a fighter), a
// marksman and a support for bot; the skirmisher is flex, eligible for a
// top or a mid seat. The seats a team already holds are consumed first (a
// fixed role takes its own seat, the flex and the unknown take the first
// seat open), then each open seat draws among the roster champions
// eligible for it that the team does not hold, from the match's Rng: the
// fill varies by seed and complements what the team has, so a lone marksman
// gets a support beside them. A seat with nobody eligible left draws among
// whatever is unused. Duplicates are forbidden inside a team only; the
// other team may hold the same champion. The lane each one then plays is
// the sim's call (src/sim/lanes.ts), by the same roles.
...
type SeatKind = 'mid' | 'top' | 'carry' | 'support';

const SEATS: readonly SeatKind[] = ['mid', 'top', 'top', 'carry', 'support'];
