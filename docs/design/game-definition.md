...
- **Map**: the Star Orchard (`docs/star-orchard.md`, ADR 0021): two spawn platforms, three lanes of three towers each plus two Sanctum guardians a base, two forests with three camps each, a central plaza. Win condition: destroy the Sanctum guardians, then the Sanctum. The launch map the game opened on (2 towers per lane, brush, chokes; `tests/tower_approach.test.ts`) is the tests' fixture now.
