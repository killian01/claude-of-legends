...
- **Access**: browser, online-first. A free account (unique name, password, email address) for anything that reaches the server; the offline practice match needs none. The address confirms the account and resets a forgotten password, and nothing else (ADR 0007). No database: JSON files on disk (ADR 0006).
